...
import {
  setCollecteData,
  setLoading as setCollecteLoading,
  setError as setCollecteError,
} from "../store/slices/collecteAdminSlice";
...
function CollecteAdminView() {
  ...
  const { listData, agents, isLoading, error } = useAppSelector(
    (s) => s.collecteAdmin,
  );
    ...
    <div className="px-4 pt-6 space-y-6 animate-in fade-in duration-700 pb-24">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-on-surface-variant font-medium mt-0.5">
            {totalRecords} collectes enregistrées · triées par agent le plus
            actif
          </p>
        </div>
      </div>
      ...
      <section className="space-y-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
          {agents.map((agent, idx) => (
            ...
            >
              ...
              <div className="flex-shrink-0 w-9 h-9 rounded-full bg-surface-container flex items-center justify-center">
                {idx < 3 ? (
                  <span
                    className={`material-symbols-outlined text-lg ${medalColors[idx]}`}
                    style={{ fontVariationSettings: "'FILL' 1" }}
                  >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-black text-on-surface text-sm truncate">
                  {agent.agent_name}
                </p>
                <p className="text-[10px] text-on-surface-variant font-medium">
                  {agent.nb_collectes} collecte
                  {agent.nb_collectes !== 1 ? "s" : ""}
                </p>
              </div>
              {/* Amount */}
          ...
          {agents.length === 0 && (
            <p className="text-sm text-on-surface-variant col-span-full text-center py-4">
              Aucun agent actif
            </p>
          )}
        </div>
            ...
            <tbody className="divide-y divide-outline-variant/5">
              {isLoading ? (
                <tr>
                  <td
                    colSpan={4}
                    className="px-5 py-10 text-center animate-pulse"
                  >
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        ...
                        <p className="text-sm font-black text-on-surface">
                          ...
                          <span className="text-[9px] opacity-60">FC</span>
                        </p>
                      </td>
                      <td className="px-5 py-4">
...
function CollecteAgentView() {
  ...
  const [selectedTaxeId, setSelectedTaxeId] = useState<number | null>(
    taxes[0]?.id || null,
  );
  ...
  const [selectedVendeurId, setSelectedVendeurId] = useState<number | null>(
    null,
  );
  const [searchQuery, setSearchQuery] = useState("");
  ...
  const fetchVendeurs = useCallback(
    async (query: string, page: number, taxId?: number) => {
      setLoading(true);
      try {
        const data = await vendeurApi.getListView(
          query,
          page * PAGE_SIZE,
          PAGE_SIZE,
          taxId,
        ...
        setModal({
          kind: "error",
          message: err.message || "Erreur lors du chargement des vendeurs.",
        });
      } finally {
        setLoading(false);
      }
  ...
  const handleRegisterPayment = async () => {
    if (!selectedVendeurId || !selectedTaxeId || !selectedTaxe) {
      setModal({
        kind: "error",
        message: "Veuillez sélectionner un vendeur et une taxe.",
      });
      return;
    }

    const finalAmount = selectedTaxe.prix_libre
      ? Number(customAmount)
      : selectedTaxe.montant_base;
    if (selectedTaxe.prix_libre && (!finalAmount || finalAmount <= 0)) {
      setModal({
        kind: "error",
        message:
          "Veuillez saisir un montant valide pour cette taxe occasionnelle.",
      });
      return;
    }
    ...
    try {
      ...
      await paiementApi.create({
        ...
        montant: finalAmount,
        reference,
        date_paiement: selectedDate
          ? new Date(selectedDate + "T00:00:00").toISOString()
          : undefined,
      });
      setModal({
        kind: "success",
        message: `Le paiement de ${finalAmount.toLocaleString("fr-FR")} FC pour ${selectedVendeur?.noms ?? "ce vendeur"} a été enregistré.`,
      });
  ...
  return (
    <div className="flex flex-col items-center px-4 pt-6 space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-700 pb-32">
      {/* Search Bar */}
      <div className="relative flex items-center gap-2 w-full lg:w-[80%]">
      ...
      {/* Vendor List */}
      <section className="space-y-4 w-full lg:w-[80%]">
        <div className="flex justify-between items-center px-1">
          <h3 className="text-[11px] font-black text-on-surface-variant uppercase tracking-[0.2em]">
        ...
        <div className="overflow-x-auto rounded-3xl border border-outline-variant/10 shadow-sm bg-surface-container-lowest">
                ...
                vendeurs.map((v) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      {v.derniere_collecte ? (
                        <div>
                          ...
                          <p className="text-[9px] text-on-surface-variant uppercase font-black">
                            Payé
                          </p>
                        </div>
                      ) : (
                    ...
                    <td className="px-5 py-4 text-center">
                      <div
                        className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${selectedVendeurId === v.id ? "bg-primary border-primary" : "border-outline-variant/30"}`}
                      >
          ...
          <div className="flex justify-between items-center p-4 bg-surface-container-lowest border-t border-outline-variant/10">
            ...
            <Button
              variant="outline"
              size="sm"
              disabled={vendeurs.length < PAGE_SIZE || loading}
              onClick={() => setCurrentPage((prev) => prev + 1)}
            >
              Suivant{" "}
              <span className="material-symbols-outlined text-sm ml-1">
      ...
      {/* Date Selection */}
      <section className="space-y-4 w-full lg:w-[80%] px-1">
        <h3 className="text-[11px] font-black text-on-surface-variant uppercase tracking-[0.2em]">
          Date de la Collecte
        </h3>
        <div className="bg-surface-container-lowest p-5 rounded-3xl border border-outline-variant/10 shadow-sm flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-[10px] text-on-surface-variant font-medium mt-0.5">
              La date est initialisée à la dernière collecte pour faciliter le
              suivi, mais vous pouvez utiliser aujourd'hui.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {taxes.map((taxe) => (
            <div
              key={taxe.id}
              onClick={() => {
                setSelectedTaxeId(taxe.id);
                if (selectedTaxeId !== taxe.id) setCustomAmount("");
              }}
            ...
            >
              <div
                className={`absolute top-4 right-4 w-6 h-6 rounded-full flex items-center justify-center transition-all ${selectedTaxeId === taxe.id ? "bg-primary text-white scale-100" : "bg-surface-container border border-outline-variant/30 scale-0"}`}
              >
              ...
              <div
                className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 transition-colors ${selectedTaxeId === taxe.id ? "bg-primary text-white" : "bg-primary/10 text-primary"}`}
              >
                <span className="material-symbols-outlined text-2xl">
                  {taxe.frequence.includes("Journal")
                    ? "calendar_today"
                    : taxe.frequence.includes("Mensu")
                      ? "event"
                      : taxe.frequence.includes("Annu")
                        ? "calendar_month"
                        : "bolt"}
                </span>
              </div>
              <h5
                className={`font-black tracking-tight transition-colors ${selectedTaxeId === taxe.id ? "text-primary" : "text-on-surface"}`}
              >
                {taxe.nom}
              </h5>
              ...
              {!taxe.prix_libre && (
                <div className="mt-2 text-right">
                  <p
                    className={`text-xl font-black leading-none ${selectedTaxeId === taxe.id ? "text-primary" : "text-on-surface"}`}
                  >
                    {taxe.montant_base.toLocaleString("fr-FR")}
                  </p>
                  <p className="text-[9px] font-black opacity-60 uppercase tracking-widest mt-1">
                    CDF
                  </p>
                </div>
              )}
              {taxe.prix_libre && (
                <div className="mt-2 text-right">
                  <p
                    className={`text-xs font-bold italic ${selectedTaxeId === taxe.id ? "text-primary" : "text-on-surface-variant"}`}
                  >
                    Prix libre
                  </p>
                </div>
      ...
      {selectedTaxe?.prix_libre && (
        <section className="w-full lg:w-[80%] px-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="bg-surface-container-lowest p-5 rounded-3xl border border-outline-variant/10 shadow-sm space-y-3">
            ...
            <p className="text-[10px] text-on-surface-variant/60 font-medium px-1 italic">
              Cette taxe est à prix libre. Le montant doit être saisi
              manuellement.
            </p>
          </div>
        </section>
      ...
      {/* Action Button */}
      <div className="w-full lg:w-[50%] px-4 transform transition-all hover:-translate-y-1">
        <Button
          variant="primary"
          fullWidth
          size="lg"
        ...
        >
          ...
          {selectedVendeur
            ? `Enregistrer pour ${selectedVendeur.noms.split(" ")[0]}`
            : "Sélectionner un vendeur"}
        </Button>
      </div>
    </div>
