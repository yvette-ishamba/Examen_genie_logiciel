  ...
  const handleExportPDF = () => {
    ...
    if (complianceChartRef.current) {
      ...
      reports.compliance_stats.forEach((stat, i) => {
        doc.text(`${stat.label}: ${stat.percentage?.toFixed(1)}% (${stat.value} vendeurs)`, 90, yPos + 20 + (i * 10));
      });
      yPos += 70;
    }
    ...
    if (categoryChartRef.current) {
      ...
      reports.revenue_by_category.forEach((cat, i) => {
        doc.text(`${cat.label}: ${cat.value.toLocaleString()} FC`, 110, yPos + 30 + (i * 10));
      });
      yPos += 90;
    }
    ...
    doc.text("Note: Ce rapport est généré automatiquement par le système Congo Tax App.", 20, yPos + 10);
    doc.text("Il est destiné à l'usage exclusif de l'Autorité Locale pour la prise de décision.", 20, yPos + 18);

    doc.save(`Rapport_Decisionnel_${new Date().toISOString().split('T')[0]}.pdf`);
  };
  ...
  if (error || !reports) {
    return (
      <div className="p-10 text-center">
        ...
        <p className="text-on-surface-variant">{error || "Impossible de charger les données"}</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-8 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-4xl font-black text-primary tracking-tight">Rapports Décisionnels</h1>
          <p className="text-on-surface-variant font-medium">Analyse globale de l'écosystème fiscal de la juridiction.</p>
        </div>
        <div className="flex gap-3">
          <Button variant="outline" className="rounded-2xl font-black py-3 px-6 shadow-sm">
            <FaFilter className="w-4 h-4 mr-2" /> Filtres Avancés
          </Button>
          <Button 
            variant="primary" 
        ...
        <div className="bg-white p-8 rounded-[32px] border border-outline-variant/10 shadow-sm flex flex-col h-full group hover:shadow-xl transition-all duration-500">
          <div className="flex justify-between items-start mb-8">
            <div>
              ...
              <h2 className="text-xl font-black text-on-surface tracking-tight">Distribution par Marché</h2>
              <p className="text-xs text-on-surface-variant font-bold uppercase tracking-widest mt-1">Recettes cumulées</p>
            </div>
        ...
        <div className="bg-white p-8 rounded-[32px] border border-outline-variant/10 shadow-sm flex flex-col h-full group hover:shadow-xl transition-all duration-500">
          <div className="flex justify-between items-start mb-8">
            <div>
              ...
              <h2 className="text-xl font-black text-on-surface tracking-tight">Taux de Conformité</h2>
              <p className="text-xs text-on-surface-variant font-bold uppercase tracking-widest mt-1">État des paiements journaliers</p>
            </div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 flex-1">
            ...
            <div className="w-full md:w-1/2 space-y-4">
              {reports.compliance_stats.map((stat, i) => (
                <div key={i} className="bg-surface-container-lowest p-4 rounded-2xl border border-outline-variant/5">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xs font-black text-on-surface uppercase">{stat.label}</span>
                    <span className="text-xs font-black text-primary">{stat.percentage?.toFixed(1)}%</span>
                  </div>
        ...
        <div className="bg-white p-8 rounded-[32px] border border-outline-variant/10 shadow-sm flex flex-col h-full group hover:shadow-xl transition-all duration-500">
          <div className="flex justify-between items-start mb-8">
            <div>
              ...
              <h2 className="text-xl font-black text-on-surface tracking-tight">Performance des Agents</h2>
              <p className="text-xs text-on-surface-variant font-bold uppercase tracking-widest mt-1">Top 30 derniers jours</p>
            </div>
          </div>

          <div className="space-y-4 flex-1">
            {reports.agent_performance.length === 0 ? (
              <p className="text-center py-10 text-on-surface-variant italic text-sm">Aucune activité enregistrée.</p>
            ) : (
              reports.agent_performance.slice(0, 5).map((agent, i) => (
                <div key={i} className="flex items-center justify-between p-4 bg-surface-container-low rounded-2xl group/item hover:bg-primary/5 transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-sm font-black text-on-surface">{agent.label}</p>
                      <p className="text-[10px] text-on-surface-variant font-bold uppercase">Collecteur Certifié</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-black text-primary">{agent.value.toLocaleString()} FC</p>
                    <p className="text-[10px] text-secondary font-black uppercase">Performant</p>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="bg-white p-8 rounded-[32px] border border-outline-variant/10 shadow-sm flex flex-col h-full group hover:shadow-xl transition-all duration-500">
          <div className="flex justify-between items-start mb-8">
            <div>
              ...
              <h2 className="text-xl font-black text-on-surface tracking-tight">Recettes par Fréquence</h2>
              <p className="text-xs text-on-surface-variant font-bold uppercase tracking-widest mt-1">Répartition des flux</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
             {reports.revenue_by_category.map((cat, i) => (
               <div key={i} className="bg-surface-container-lowest p-3 rounded-xl border border-outline-variant/5">
                 <p className="text-[10px] font-black text-on-surface-variant uppercase mb-1">{cat.label}</p>
                 <p className="text-sm font-black text-on-surface">{cat.value.toLocaleString()} <span className="text-[9px] opacity-40">FC</span></p>
               </div>
             ))}
          </div>
        </div>
      ...
      <div className="bg-primary/5 border border-primary/10 p-8 rounded-[32px] flex items-start gap-6">
        ...
        <div>
          <h3 className="text-xl font-black text-primary mb-2 tracking-tight">Note d'Aide à la Décision</h3>
          <p className="text-sm text-on-surface-variant font-medium leading-relaxed max-w-4xl">
            Sur la base des données actuelles, le marché avec la plus forte contribution est <strong>{reports.market_distribution[0]?.label || "N/A"}</strong>. 
            Le taux de conformité global est de <strong>{reports.compliance_stats[0]?.percentage?.toFixed(1)}%</strong>. 
            Une attention particulière devrait être portée aux taxes de type <strong>{[...reports.revenue_by_category].sort((a,b) => a.value - b.value)[0]?.label || "N/A"}</strong> qui présentent le volume de recettes le plus faible.
          </p>
        </div>
      </div>
    </div>
